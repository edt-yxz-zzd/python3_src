...
zero_poly_K0K1K2GG_of_Cs_half_half_half = (-GG*K0**4 + 2*GG*K0**2*K1**2 + 2*GG*K0**2*K2**2 - GG*K1**4 + 2*GG*K1**2*K2**2 - GG*K2**4 + 4*K0**4 + 4*K0**2*K1**2*K2**2 - 8*K0**2*K1**2 - 8*K0**2*K2**2 + 4*K1**4 - 8*K1**2*K2**2 + 4*K2**4)
...
def calc_zero_poly_K0K1K2W_of_Cs_half_half_half():
    ...
    solid2g_k0k1k2 = {
        ...
        (4, 6, 10): (0.2629921750726, (sqrt(2), sqrt(3), sqrt(sqrt(5) + 5)/sqrt(2))),
        }
...
def get_zero_poly_K0K1K2GG_of_Cs_half_half_half():
    f = zero_poly_K0K1K2W_of_Cs_half_half_half.subs({K0W: K0*W, K1W: K1*W, K2W: K2*W})
    f = expand(f/W**4)
    h = f.subs(W*W, 1/(4-GG))
    h *= (4-GG)
    h = expand(factor(h))
    assert h == zero_poly_K0K1K2GG_of_Cs_half_half_half
    return h
