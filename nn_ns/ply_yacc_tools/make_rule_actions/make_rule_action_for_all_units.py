...
def make_rule_action_for_all_units(
    locals, unit_rules_in_str, *, transform=None
    ):
    '''
...
def is_true_unit_rule(true_tuple_rule):
    ...
    L = len(alternatives)
    if L == 0:
        raise ValueError('len(alternatives) == 0 ==>> dead')
        return False

    return all(map(is_true_unit_alternative, alternatives))
def is_true_unit_alternative(alternative):
    # count True
    L = len(alternative)
    pairs = alternative # [(bool, name)]
    fsts = [fst for fst, _ in pairs]
    num_Trues = sum(fst is True for fst in fsts)
    num_Falses = sum(fst is False for fst in fsts)
...
def basic_make_rule_action_for_all_units(
...
'''
    ...
    for name, alternatives in true_tuple_rules:
        ...
        def p_(p, *, i=i, transform=transform):
            p[0] = transform(p[i])
