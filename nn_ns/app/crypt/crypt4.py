...
changes:
    ...
    version2 -> version3:
        ...
        add uint2uint_by_invmod # too slow
            by now, encrypt 3000 chars require minutes...
    version3.1
        fix bug
            #bug:for digit in sized_randoms(L, 0, upper_len):
            #bug:if not length: break
            #bug:yield from block
        speedup uint2uint_by_invmod
            UInt2UInt_by_mod using InvModPow instead of invmod

    version3 -> version4
        remove iter_message_with_psw/iter_ciphertext_with_psw
        add iter_message_with_psw_digest/iter_ciphertext_with_psw_digest
            # hence we donot use psw directly
        # 10KB -> 0.5MINUTE

TODO
    * zip input??
        but input should have no format
...
import hashlib
from seed.int_tools.uint_bits_utils import uint2bytes as _uint2bytes
...
class BijectionsImpl:
    ...
    def add_noise(radix, digits, upper_len):
        ...
        assert B >= 0
        digits = tuple(digits)
        lens = randoms(0, upper_len)

        def iter_escape_digits(digits):
            return itertools.chain.from_iterable(map(iter_escape_digit, digits))
        def iter_escape_digit(digit):
            if not 0 <= digit < D: raise TypeError
            if digit == E:
        ...
        def iter_comment(*, without_close=False):
            ...
            #bug:for digit in sized_randoms(L, 0, upper_len):
            for digit in sized_randoms(L, 0, D):
                yield from iter_escape_digit(digit)
        ...
        for block in blocks:
            #bug:yield from block
            yield from iter_escape_digits(block)
            break
        for block in blocks:
            yield from next(comments)#no raise
            #bug:yield from block
...
class Bijections:
    ...
    def digits2uint(radix, digits, *, little_endian:bool):
        if __debug__:
            digitsI = list(digits); del digits
            digits = iter(digitsI)
        u = BijectionsImpl.digits2uint(radix, digits, little_endian=little_endian)
        if __debug__:
            assert  [*BijectionsImpl.uint2digits(radix, u, little_endian=little_endian)] == digitsI
        return u
    @staticmethod
    def uint2digits(radix, uint, *, little_endian:bool):
        digits = BijectionsImpl.uint2digits(radix, uint, little_endian=little_endian)
        if __debug__:
            digitsO = list(digits); del digits
            digits = iter(digitsO)
            assert BijectionsImpl.digits2uint(radix, digits, little_endian=little_endian) == uint
            digits = iter(digitsO)
        return iter(digits)
    @staticmethod
    def int2uint(i):
        u = BijectionsImpl.int2uint(i)
        assert BijectionsImpl.uint2int(u) == i
        return u
    @staticmethod
    def uint2int(u):
        i = BijectionsImpl.uint2int(u)
        assert BijectionsImpl.int2uint(i) == u
        return i
    @staticmethod
    def pad_uint(pad, uint):
        u_ = BijectionsImpl.pad_uint(pad, uint)
        assert BijectionsImpl.pad_uint(-pad, u_) == uint
        return u_
    ...
    def uint2uint_by_mod(odd_K, A, uint, *, backward:bool, inv:bool=False):
        ...
        inv = bool(inv)
        u_ = BijectionsImpl.uint2uint_by_mod(odd_K, A, uint, backward=backward, inv=inv)
        assert BijectionsImpl.uint2uint_by_mod(odd_K, A, u_, backward=not backward, inv=inv) == uint
        return u_

    @staticmethod
    def uint2uint_by_invmod(odd_K, A, uint, *, backward:bool):
        backward = bool(backward)
        u_ = BijectionsImpl.uint2uint_by_invmod(odd_K, A, uint, backward=backward)
        assert BijectionsImpl.uint2uint_by_invmod(odd_K, A, u_, backward=not backward) == uint
        return u_

    @staticmethod
    def remove_noise(radix, digits):
        # not bijection
        # remove_noise . add_noise == id
        # add_noise. remove_noise != id
        return BijectionsImpl.remove_noise(radix, digits)
    @staticmethod
    def add_noise(radix, digits, upper_len):
        if __debug__:
            digitsI = list(digits); del digits
            digits = iter(digitsI)
        digits = BijectionsImpl.add_noise(radix, digits, upper_len)
        if __debug__:
            digitsO = list(digits); del digits
            try:
                assert [*BijectionsImpl.remove_noise(radix, digitsO)] == digitsI
            except:
                print('radix =', radix)
                print('digitsI =', digitsI)
                print('digitsO =', digitsO)
                raise
            digits = iter(digitsO)
        return iter(digits)


def len_same_prefix(iterL, iterR):
    for i, (a, b) in enumerate(zip(iterL, iterR)):
        if a != b: return i
    return 0

def _debug_add_noise():
    from .tmp__add_noise__error_output import radix, digitsI, digitsO
    add_noise = BijectionsImpl.add_noise
    remove_noise = lambda *args: list(BijectionsImpl.remove_noise(*args))
    [*digitsI_] = remove_noise(radix, digitsO)
    len_prefix = len_same_prefix(digitsI, digitsI_)
    #print(len_prefix); print(digitsI[len_prefix:]); print(digitsI_[len_prefix:])

    i,j = 0, len(digitsO)
    while i < j:
        mid = (i+j)//2
        ls = remove_noise(radix, digitsO[:mid])
        if len(ls) < len_prefix:
            i = mid+1
        else:
            j = mid
    assert i == j
    assert len(remove_noise(radix, digitsO[:i])) == len_prefix
    assert i == 0 or len(remove_noise(radix, digitsO[:i-1])) < len_prefix
    print(len_prefix); print(i)
    print('digitsI[len_prefix:] =', digitsI[len_prefix:])
    print('digitsO[i:] =', digitsO[i:])
#_debug_add_noise(); del _debug_add_noise
...
def uint2bytesLE(u):
    byteorder = 'little'
    bs = _uint2bytes(u, byteorder=byteorder)
    assert u == int.from_bytes(bs, byteorder=byteorder)
    assert (bs == b'') if u == 0 else bs[-1]
    return bs
def _test_uint2bytesLE():
    for u in range(10):
        uint2bytesLE(u)
    uint2bytesLE(0xFF)
    uint2bytesLE(0x1FF)
    uint2bytesLE(0x100)
    uint2bytesLE(0x101)
_test_uint2bytesLE(); del _test_uint2bytesLE
...
class CryptPrime:
    ...
    def iter_infinite_psw(self, psw):
        ...
        W = self.len_psw_charset
        if not all(0 <= w < W for w in psw): raise TypeError
    ...
    def iter_digits_with_psw(self, psw, radix, digits, *, add:bool):
        ...
        for d, w in zip(digits, self.iter_infinite_psw(psw)):
            if not 0 <= d < D: raise TypeError
            yield f(d, w)%D
        pass
    '''
    def iter_message_with_psw_digest(self, psw, message, *, add:bool):
        radix = self.len_input_charset
        digits = message
        hash_salt = __class__.SALT_I
        return self.iter_digits_with_psw_digest(psw, radix, digits
                    , add=add, hash_salt=hash_salt)
    def iter_ciphertext_with_psw_digest(self, psw, ciphertext, *, add:bool):
        radix = self.len_output_charset
        digits = ciphertext
        hash_salt = __class__.SALT_O
        return self.iter_digits_with_psw_digest(psw, radix, digits
                    , add=add, hash_salt=hash_salt)

    def iter_infinite_psw_digest(self, psw, *, hash_salt:bytes):
        len(psw)
        W = self.len_psw_charset
        u = Bijections.digits2uint(W, psw, little_endian=True)
        #not bijection, though not required:
        #   bs0 = uint2bytes(u, byteorder='big')
        bs0 = Bijections.uint2digits(0x100, u, little_endian=False)
        bs0 = bytes(bs0) + hash_salt
        step = u%419 + 1
        del u
        ...
        def make_digest(bs):
            ...
            k = -1
            for k in range(m.digest_size//3):
                if digest[k]: break
            else:
                k += 1
            digest = digest[k:]
        ...
        for x in itertools.count(30+step, step):
            bs = uint2bytesLE(x)
            digest = make_digest(bs)
            yield from digest

    def iter_infinite_psw_digest_in_radix(self, psw, radix, *, hash_salt:bytes):
        D = radix
        bs = self.iter_infinite_psw_digest(psw, hash_salt=hash_salt)
        assert iter(bs) is bs
        L = 1+  D.bit_length()//8 # >= D.byte_length
        for b in bs:
            u = int.from_bytes(bytes(itertools.islice(bs, L)), byteorder='big')
            u += b
            u %= D
            yield u
        return
    def iter_digits_with_psw_digest(self, psw, radix, digits, *, add:bool, hash_salt):
        D = radix
        f = __add__ if add else __sub__
        ds = self.iter_infinite_psw_digest_in_radix(psw, radix, hash_salt=hash_salt)
        for d, d2 in zip(digits, ds):
            if not 0 <= d < D: raise TypeError
            yield f(d, d2)%D
        return
    ...
    def encrypt(self, psw, message):
        ...
        digitsI = self.iter_message_with_psw_digest(psw, noised, add=True)
        #[*digitsI] = digitsI
        ...
        ciphertext = self.iter_ciphertext_with_psw_digest(psw, digitsO, add=True)
    ...
    def decrypt(self, psw, ciphertext):
        ...
        digitsO = self.iter_ciphertext_with_psw_digest(psw, ciphertext, add=False)
        #[*digitsO] = digitsO
        radixO = self.len_output_charset
        ...
        noised = self.iter_message_with_psw_digest(psw, digitsI, add=False)
...
def random_split(seq, min_len, upper_len):
    ...
    L = len(seq)
    i = 0
    lens = randoms(min_len, upper_len)
    while i < L:
        length = next(lens)
        assert length > 0
        j = i + length
...
class Crypt:
    ...
    def encrypt(self, psw, message):
        ...
        assert type(message) is type(ciphertext) is type(message2) is str
        return ciphertext
...
def insert_spaces(ciphertext):
    # insert ' ' and '\n'
    if len(ciphertext.split()) > 1: raise TypeError('should not contains spaces')
    words = random_split(ciphertext, 1, 8)
    lines = random_split(words, 1, 8)
    lines = '\n'.join(' '.join(words) for words in lines)
    if remove_spaces(lines) != ciphertext: raise logic-error
