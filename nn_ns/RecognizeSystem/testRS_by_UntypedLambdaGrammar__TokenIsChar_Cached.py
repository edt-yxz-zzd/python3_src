...
from .RecognizeSystem import CachedStream
from .RecognizeSystem__TokenIsChar import CachedRecognizeSystem__TokenIsChar
...
class UntypedLambdaCompiler(CachedRecognizeSystem__TokenIsChar):
...
def compileUntypedLambda(s, recognizer=start_symbol_recognizer):
    st = ArrayStream(OffsetedArray(s))
    st = CachedStream(st)
    v, _ = recognizer(st)
    return v
def compileUntypedLambdaEx(s, id):
    recognizer = compilerUntypedLambda.id2recognizer(id)
    return compileUntypedLambda(s, recognizer)
