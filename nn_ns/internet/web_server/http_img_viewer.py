...
import os
#import urllib.request as URL #PY3 
import urllib as URL #PY2
...
chapter_page_link_tpl = r'<img src="file:{link}" alt="{name}" width="100%">'
...
class HomePage(Resource):
    ...
    def make_link(self, name):
        return self.link_tpl.format(link = self.prefix + '/' + name, name = name)
    ...
    def render_GET(self, request):
        ...
        links = []
        for name in names:
            links.append(self.make_link(name))
...
class ChapterPage(HomePage):
    ...
    def make_link(self, name):
        ...
        link = URL.pathname2url(fname)
        
        return self.link_tpl.format(link = link, name = name)
