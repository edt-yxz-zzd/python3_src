...
def extract_360doc_com(html_file):
    # html_file may be BytesIO/StringIO?
    soup = BeautifulSoup(html_file, 'lxml')
    title_h2 = soup.find(id='titiletext') # not titletext!!!
    if title_h2 is None: NotFoundError('title')
    title = title_h2.get_text()
    content_div = soup.find(id="articlecontent")
    if content_div is None: NotFoundError('content')
...
'''
http://www.360doc.com/content/12/0203/08/700052_183801900.shtml
