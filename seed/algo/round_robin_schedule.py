#__all__:goto
r'''[[[
e ../../python3_src/seed/algo/round_robin_schedule.py
    round_robin_schedule
    循环赛配对方案
构建冫循环赛配对方案冖冖易用接口扌
py_adhoc_call   seed.algo.round_robin_schedule   ,str.构建冫循环赛配对方案冖冖易用接口扌  +欤标准压缩 +欤紧凑文本化  --版本名:非递归冫惑缺一轮奇位移辻惑偶偏心偶中心对称版  --队伍数半数=100

[[
.+1,$s/script[.]循环对抗赛配对方案/seed.algo.round_robin_schedule/g
.+1,$s/循环对抗赛配对方案冖冖规则化/循环赛配对方案/g
...
###py_adhoc_call   seed.algo.round_robin_schedule   ,蛮力搜索冫循环对抗赛配对方案冖冖最小化扌  --队伍数半数=3
py_adhoc_call   seed.algo.round_robin_schedule   ,构建冫循环赛配对方案冖冖尾二扌  --队伍数半数=3
py_adhoc_call   seed.algo.round_robin_schedule   ,枚举冫循环赛配对方案扌 +欤扁平化冫配对方案 +欤输出冫队伍数总数 --版本名:尾二版  --鬽队伍数半数纟最大许可=3
py_adhoc_call   seed.algo.round_robin_schedule   ,枚举冫循环赛配对方案扌 +欤扁平化冫配对方案 +欤输出冫队伍数总数 --版本名:平分版  --鬽队伍数半数纟最大许可=9
py_adhoc_call   seed.algo.round_robin_schedule   ,枚举冫循环赛配对方案扌 +欤扁平化冫配对方案 +欤输出冫队伍数总数 +欤不输出冫奇数的队伍数半数 --版本名:非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数版  --鬽队伍数半数纟最大许可=9
#失败#py_adhoc_call   seed.algo.round_robin_schedule   ,枚举冫循环赛配对方案扌 +欤扁平化冫配对方案 +欤输出冫队伍数总数 --版本名:非递归冫交错位移日字绕圈版  --鬽队伍数半数纟最大许可=9
py_adhoc_call   seed.algo.round_robin_schedule   ,str.构建冫循环赛配对方案冖冖易用接口扌  +欤标准压缩 +欤紧凑文本化  --版本名:非递归冫惑缺一轮奇位移辻惑偶偏心偶中心对称版  --队伍数半数=100
...
[[[
===
$ py_adhoc_call   seed.algo.round_robin_schedule   ,蛮力搜索冫循环对抗赛配对方案冖冖最小化扌  --队伍数半数=1
[1, 0]
$ py_adhoc_call   seed.algo.round_robin_schedule   ,蛮力搜索冫循环对抗赛配对方案冖冖最小化扌  --队伍数半数=2
[1, 0, 3, 2]
[2, 3, 0, 1]
[3, 2, 1, 0]
$ py_adhoc_call   seed.algo.round_robin_schedule   ,蛮力搜索冫循环对抗赛配对方案冖冖最小化扌  --队伍数半数=3
...
[4, 3, 5, 1, 0, 2]
[5, 2, 1, 4, 3, 0]
$ py_adhoc_call   seed.algo.round_robin_schedule   ,蛮力搜索冫循环对抗赛配对方案冖冖最小化扌  --队伍数半数=4
[1, 0, 3, 2, 5, 4, 7, 6]
[2, 3, 0, 1, 6, 7, 4, 5]
[3, 2, 1, 0, 7, 6, 5, 4]
[4, 5, 6, 7, 0, 1, 2, 3]
...
[6, 7, 4, 5, 2, 3, 0, 1]
[7, 6, 5, 4, 3, 2, 1, 0]
$ py_adhoc_call   seed.algo.round_robin_schedule   ,蛮力搜索冫循环对抗赛配对方案冖冖最小化扌  --队伍数半数=5
[1, 0, 3, 2, 5, 4, 7, 6, 9, 8]
[2, 3, 0, 1, 6, 8, 4, 9, 5, 7]
[3, 2, 1, 0, 7, 9, 8, 4, 6, 5]
[4, 5, 6, 9, 0, 1, 2, 8, 7, 3]
...
[8, 9, 4, 6, 2, 7, 3, 5, 0, 1]
[9, 6, 5, 7, 8, 2, 1, 3, 4, 0]
$ py_adhoc_call   seed.algo.round_robin_schedule   ,蛮力搜索冫循环对抗赛配对方案冖冖最小化扌  --队伍数半数=7
...
$ py_adhoc_call   seed.algo.round_robin_schedule   ,蛮力搜索冫循环对抗赛配对方案冖冖最小化扌  --队伍数半数=9
...
[[[
===
py_adhoc_call   seed.algo.round_robin_schedule   ,枚举冫循环赛配对方案冖冖尾二扌  --鬽队伍数半数纟最大许可=9 +欤扁平化冫配对方案 +欤输出冫队伍数总数
===
>>> def print_iter(it, /):
...     for x in it:print(x)
>>> print_iter(枚举冫循环赛配对方案冖冖尾二扌(鬽队伍数半数纟最大许可=9, 欤扁平化冫配对方案=True, 欤输出冫队伍数总数=True))
2
[1, 0]
4
[1, 0, 3, 2]
...
===
>>> print_iter(枚举冫循环赛配对方案扌(版本名='蛮力最小化版', 鬽队伍数半数纟最大许可=9, 欤扁平化冫配对方案=True, 欤输出冫队伍数总数=True, 欤标准压缩=True))
2
4
[[0, 2, 1, 3]]
6
...
[:二轮定理:循环赛配对方案中任意两轮赛事交错成环丶环长为偶数丶环长大于等于四]:goto
>>> print_iter(枚举冫循环赛配对方案扌(版本名='蛮力最小化版', 鬽队伍数半数纟最大许可=9, 欤扁平化冫配对方案=True, 欤输出冫队伍数总数=True, 欤标准压缩=True, 欤紧凑文本化=True))
2
4
[0,2,1,3;]
6
...
>>> 打印冫所有版本信息扌()
尾二版:+欤算法完整,-欤算法蛮力,+欤算法递归
平分版:+欤算法完整,-欤算法蛮力,+欤算法递归
蛮力最小化版:+欤算法完整,+欤算法蛮力,+欤算法递归
非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数版:-欤算法完整,-欤算法蛮力,-欤算法递归
非递归冫奇位移辻偶偏心对称乊仅限于偶数半数版:-欤算法完整,-欤算法蛮力,-欤算法递归
非递归冫惑缺一轮奇位移辻惑偶偏心偶中心对称版:+欤算法完整,-欤算法蛮力,-欤算法递归
非递归冫缺一轮奇位移辻偶中心对称乊仅限于奇数半数版:-欤算法完整,-欤算法蛮力,-欤算法递归
>>> print_iter(枚举冫循环赛配对方案扌(版本名='尾二版', 鬽队伍数半数纟最大许可=2, 欤扁平化冫配对方案=True, 欤输出冫队伍数总数=True, 欤紧凑文本化=True))
2
[[1,0]
]
4
...
...         print(f'队伍数半数:={n}')
...         try:
...             print_iter(枚举冫循环赛配对方案扌(版本名=版本名, 鬽队伍数半数纟最大许可=n, 队伍数半数纟最小许可=n, 欤扁平化冫配对方案=True, 欤紧凑文本化=True))
...
>>> def 鬽结果扌(版本名,队伍数半数,/):
...     try:
...         return 构建冫循环赛配对方案冖冖易用接口扌(版本名=版本名, 队伍数半数=队伍数半数)
...     except TypeError:
...         return None
...
===
#失败#>>> for 配对方案 in 枚举冫循环赛配对方案扌(版本名='蛮力最小化版', 鬽队伍数半数纟最大许可=9): 蛮力搜索冫只差一对的拟遍历圈纟循环对抗赛配对方案冖冖扩张预备扌(配对方案)

===
]]]
[[[
#py_adhoc_call   seed.algo.round_robin_schedule   @输出枚举冫循环赛配对方案冖冖尾二扌 +欤读取 --输出路径:./script/循环对抗赛配对方案.py.out.txt  +verbose --队伍数半数纟最大许可=6
#py_adhoc_call   seed.algo.round_robin_schedule   @输出枚举冫循环赛配对方案冖冖尾二扌 +欤读取 --输出路径:./script/循环对抗赛配对方案.py.out.txt  +verbose --队伍数半数纟最大许可=9
#py_adhoc_call   seed.algo.round_robin_schedule   @输出枚举冫循环赛配对方案冖冖尾二扌 +欤读取 --输出路径:./script/循环对抗赛配对方案.py.out.txt  +verbose --队伍数半数纟最大许可=20
#py_adhoc_call   seed.algo.round_robin_schedule   @输出枚举冫循环赛配对方案冖冖尾二扌 +欤读取 --输出路径:./script/循环对抗赛配对方案.py.out.txt  +verbose --队伍数半数纟最大许可=36
###py_adhoc_call   seed.algo.round_robin_schedule   @输出枚举冫循环赛配对方案冖冖尾二扌 +欤读取 --输出路径:./script/循环对抗赛配对方案.py.out.txt  --队伍数半数纟最大许可=256 +verbose
...
[:命令行冫输出到文件]:here
===
py_adhoc_call   seed.algo.round_robin_schedule   @输出枚举冫循环赛配对方案扌 +欤读取 --版本名:尾二版 --输出路径:./script/循环对抗赛配对方案.py.尾二版.仅奇半数.out.txt  +verbose +欤不输出冫偶数的队伍数半数 --队伍数半数纟最大许可=36
view ./script/循环对抗赛配对方案.py.尾二版.仅奇半数.out.txt
du -h ./script/循环对抗赛配对方案.py.尾二版.仅奇半数.out.txt
88KB #[队伍数半数<-[1,3..<36]]
    #删空格后
===
py_adhoc_call   seed.algo.round_robin_schedule   @输出枚举冫循环赛配对方案扌 +欤读取 --版本名:平分版 --输出路径:./script/循环对抗赛配对方案.py.平分版.仅奇半数.out.txt  +verbose +欤不输出冫偶数的队伍数半数 --队伍数半数纟最大许可=36
...
构建冫循环赛配对方案冖冖非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数扌
构建冫循环赛配对方案冖冖非递归冫惑缺一轮奇位移辻惑偶偏心偶中心对称扌
    构建冫循环赛配对方案冖冖非递归冫奇位移辻偶偏心对称乊仅限于偶数半数扌
    构建冫循环赛配对方案冖冖非递归冫缺一轮奇位移辻偶中心对称乊仅限于奇数半数扌

蛮力搜索冫循环对抗赛配对方案冖冖最小化扌
'''.split()#'''
#失败:构建冫循环赛配对方案冖冖非递归冫交错位移日字绕圈扌
#   简单位移:只能用于二幂
...
def 蛮力搜索冫循环对抗赛配对方案冖冖最小化扌(*, 队伍数半数):
    ...
    return lsls
def _get_properties_(obj, nms='欤算法完整,欤算法递归,欤算法蛮力'.split(','), /):
    return {nm:getattr(obj, nm) for nm in nms}
def _setdefault_properties_(obj, /, *, 欤算法完整, 欤算法递归, 欤算法蛮力):
    d = dict(locals())
    del d['obj']
    for nm, b in d.items():
        ...
        if m is None:
            ...
            check_type_is(bool, b)
    for nm in d:
        check_type_is(bool, getattr(obj, nm))
_setdefault_properties_(蛮力搜索冫循环对抗赛配对方案冖冖最小化扌,欤算法完整=True, 欤算法递归=True, 欤算法蛮力=True)
...
#end-class 魖循环赛配对方案冖冖分而治之(ABC):
class 乸循环赛配对方案冖冖尾二版(魖循环赛配对方案冖冖分而治之):
    r'''[[[
...
重要定理:可用于进行配对方案变换:
    类似遗传算法基因互换型变换
    任选二轮，任选它们所成交错环其一，反转连线颜色/即环上配对互换所处轮次
重要定理:可用于精简化输出:
...
def _排序冖冖非必要扌(js0, js1, /):
    ...
    return js0, js1
#end-class 乸循环赛配对方案冖冖尾二版:
class 乸循环赛配对方案冖冖平分版(魖循环赛配对方案冖冖分而治之):
    r'''[[[
...
证明:见:
    ...
    def _recur_odd(sf, 队伍数半数, /):
        ...
        ######################
    ######################
#end-class 乸循环赛配对方案冖冖平分版(魖循环赛配对方案冖冖分而治之):

_匴构造器冫尾二版 = 乸循环赛配对方案冖冖尾二版()
_匴构造器冫平分版 = 乸循环赛配对方案冖冖平分版()
...
def __():
    d = _版本名讠匴构造器
    for obj in d.values():
        _setdefault_properties_(obj, 欤算法完整=True, 欤算法递归=True, 欤算法蛮力=False)
__()

def _更新缓存冫循环赛配对方案扌(*, 版本名, 队伍数半数讠配对方案):
    匴构造器 = _版本名讠匴构造器[版本名]
    匴构造器.队伍数半数讠配对方案.update(队伍数半数讠配对方案)
    return
def 构建冫循环赛配对方案冖冖易用接口扌(*, 版本名, 队伍数半数, 欤压缩=False, 欤标准压缩=False, 欤紧凑文本化=False):
    配对方案 = 构建冫循环对抗赛配对方案巛变量环境扌(locals())
    return 配对方案
def 构建冫循环对抗赛配对方案巛变量环境扌(变量环境, /, **_kwds):
    return _调用函数巛变量环境扌(构建冫循环赛配对方案扌, _nms4arg4构建冫循环赛配对方案扌, 变量环境, **_kwds)
_nms4arg4构建冫循环赛配对方案扌='版本名,队伍数半数,欤压缩,欤标准压缩,欤紧凑文本化'.split(',')
def 构建冫循环赛配对方案扌(*, 版本名, 队伍数半数, 欤压缩, 欤标准压缩, 欤紧凑文本化):
...
def 枚举冫循环赛配对方案扌(*, 版本名, 鬽队伍数半数纟最大许可, 欤输出冫队伍数总数=False, 欤扁平化冫配对方案=False, 队伍数半数纟最小许可=1, 欤不输出冫奇数的队伍数半数=False, 欤不输出冫偶数的队伍数半数=False, 欤不输出冫非二幂的队伍数半数=False, 欤不输出冫非二幂加一的队伍数半数=False, 欤压缩=False, 欤标准压缩=False, 欤紧凑文本化=False):
    欤分割成行 = 欤扁平化冫配对方案 and 欤紧凑文本化 and not (欤标准压缩 or 欤压缩)
    it = 枚举冫队伍数半数巛变量环境扌(locals())
    for 队伍数半数 in it:
        配对方案 = 构建冫循环赛配对方案扌(版本名=版本名, 队伍数半数=队伍数半数, 欤压缩=欤压缩, 欤标准压缩=欤标准压缩, 欤紧凑文本化=欤紧凑文本化)
        队伍数总数 = 队伍数半数*2
        if 欤输出冫队伍数总数:
            yield 队伍数总数
        if 欤扁平化冫配对方案:
...
def 输出枚举冫循环赛配对方案扌(*, 版本名, 欤读取, 输出路径, 队伍数半数纟最大许可, 欤不输出冫奇数的队伍数半数=False, 欤不输出冫偶数的队伍数半数=False, 欤不输出冫非二幂的队伍数半数=False, 欤不输出冫非二幂加一的队伍数半数=False, encoding='utf8', verbose=False, 欤压缩=False, 欤标准压缩=False):
    #, 欤紧凑文本化=False???True
    ...
    if not 版本名 in _版本名讠非递归构造器:
        _更新缓存冫循环赛配对方案扌(版本名=版本名, 队伍数半数讠配对方案=d)
    ...
    with open(输出路径, 'at', encoding=encoding) as ofile:
        ...
        for 队伍数半数 in it:
            if verbose:
                print_err(f':>>{队伍数半数}')
            配对方案 = 构建冫循环赛配对方案扌(版本名=版本名, 队伍数半数=队伍数半数, 欤压缩=欤压缩, 欤标准压缩=欤标准压缩)
            #队伍数总数 = 队伍数半数*2
            fprint(f',{队伍数半数}:')
...
def 紧凑文本化冫配对方案扌(配对方案, /):
    ...
    txt = ofile.getvalue()
    return txt
def 构建冫循环赛配对方案冖冖尾二扌(*, 队伍数半数, 欤压缩=False, 欤标准压缩=False, 欤紧凑文本化=False):
    配对方案 = 构建冫循环赛配对方案扌(版本名='尾二版', *locals())
    return 配对方案
def 枚举冫循环赛配对方案冖冖尾二扌(*, 鬽队伍数半数纟最大许可, 欤输出冫队伍数总数=False, 欤扁平化冫配对方案=False, 队伍数半数纟最小许可=1, 欤压缩=False, 欤标准压缩=False, 欤紧凑文本化=False):
    return 枚举冫循环赛配对方案扌(版本名='尾二版', **locals())
def 输出枚举冫循环赛配对方案冖冖尾二扌(*, 欤读取, 输出路径, 队伍数半数纟最大许可, 欤不输出冫奇数的队伍数半数=False, 欤不输出冫偶数的队伍数半数=False, 欤不输出冫非二幂的队伍数半数=False, 欤不输出冫非二幂加一的队伍数半数=False, encoding='utf8', verbose=False, 欤压缩=False, 欤标准压缩=False):
    #, 欤紧凑文本化=False???True
    输出枚举冫循环赛配对方案扌(版本名='尾二版', **locals())
    return
...
def __():
    #失败品:
  pass
  def 构建冫循环赛配对方案冖冖非递归冫交错位移日字绕圈扌(*, 队伍数半数):
    r'''[[[
    '直接构造:非递归'
...
def _构建冫循环赛配对方案冖冖非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数扌():
    r'''[[[
    '直接构造:非递归'
方法:简单的中心对称
...
==>>:奇数半数
    ...
    * 简单位移:配对奇偶:使用 奇位移:只跳过[位移==队伍数半数]，得(队伍数半数-1)轮
    ...
    def 乊奇位移扌(队伍数半数, 队伍数总数, 奇位移, /):
        ...
        return 队伍号讠对手队伍号
    #def main(*, 队伍数半数):
    def 部分构建冫循环赛配对方案冖冖非递归冫仅部分奇位移扌(配对方案, 队伍数半数, 列表纟奇位移, /):
        ...
        for 奇位移 in 列表纟奇位移:
            ...
            配对方案.append(队伍号讠对手队伍号)
        return#配对方案
    def 部分构建冫循环赛配对方案冖冖非递归冫仅偶中心对称部分乊仅限于奇数半数扌(配对方案, 队伍数半数, /):
        #只止于:偶中心对称部分:(队伍数半数)轮
        check_int_ge(1, 队伍数半数)
        ...
        for 偶中心 in range(0, 队伍数总数, 2):
            ...
            配对方案.append(队伍号讠对手队伍号)
        return#配对方案
    def 部分构建冫循环赛配对方案冖冖非递归冫仅偶偏心对称部分乊仅限于偶数半数扌(配对方案, 队伍数半数, /):
        ...
        for 偶偏心 in range(0, 队伍数总数纟偏心, 2):
            ...
            配对方案.append(队伍号讠对手队伍号)
        return#配对方案
    def 部分构建冫循环赛配对方案冖冖非递归冫仅部分奇中心对称扌(配对方案, 队伍数半数, 列表纟奇位移, /):
    ...
    ######################
    ######################
    def 构建冫循环赛配对方案冖冖非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数扌(*, 队伍数半数):
        #非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数版
        check_int_ge(1, 队伍数半数)
        if not 队伍数半数&1 == 0: raise TypeError('前置条件:[is_even 队伍数半数]')
        ######################
        配对方案 = []
        部分构建冫循环赛配对方案冖冖非递归冫仅偶偏心对称部分乊仅限于偶数半数扌(配对方案, 队伍数半数)
        队伍数总数 = 队伍数半数*2
        部分构建冫循环赛配对方案冖冖非递归冫仅部分奇中心对称扌(配对方案, 队伍数半数, 列表纟奇位移:=range(1, 队伍数总数, 2))
        ######################
        检查冫圆满配对方案扌(队伍数半数, 配对方案)
        return 配对方案
    #end-def main():
    def 构建冫循环赛配对方案冖冖非递归冫奇位移辻偶偏心对称乊仅限于偶数半数扌(*, 队伍数半数):
        #非递归冫奇位移辻偶偏心对称乊仅限于偶数半数版
        check_int_ge(1, 队伍数半数)
        if not 队伍数半数&1 == 0: raise TypeError('前置条件:[is_even 队伍数半数]')
        ######################
        配对方案 = []
        队伍数总数 = 队伍数半数*2
        部分构建冫循环赛配对方案冖冖非递归冫仅部分奇位移扌(配对方案, 队伍数半数, 列表纟奇位移:=range(1, 队伍数总数, 2))
        部分构建冫循环赛配对方案冖冖非递归冫仅偶偏心对称部分乊仅限于偶数半数扌(配对方案, 队伍数半数)
        ######################
        检查冫圆满配对方案扌(队伍数半数, 配对方案)
        return 配对方案
    #end-def main():
    def 构建冫循环赛配对方案冖冖非递归冫缺一轮奇位移辻偶中心对称乊仅限于奇数半数扌(*, 队伍数半数):
        #非递归冫缺一轮奇位移辻偶中心对称乊仅限于奇数半数版
        check_int_ge(1, 队伍数半数)
        if not 队伍数半数&1 == 1: raise TypeError('前置条件:[is_odd 队伍数半数]')
        ######################
        ...
        部分构建冫循环赛配对方案冖冖非递归冫仅部分奇位移扌(配对方案, 队伍数半数, 列表纟奇位移:=列表纟奇位移纟缺一轮)
        部分构建冫循环赛配对方案冖冖非递归冫仅偶中心对称部分乊仅限于奇数半数扌(配对方案, 队伍数半数)
        ######################
        检查冫圆满配对方案扌(队伍数半数, 配对方案)
        return 配对方案
    #end-def main():
    def 构建冫循环赛配对方案冖冖非递归冫惑缺一轮奇位移辻惑偶偏心偶中心对称扌(*, 队伍数半数):
        #非递归冫惑缺一轮奇位移辻惑偶偏心偶中心对称版
        check_int_ge(1, 队伍数半数)
        if 队伍数半数&1 == 0:
            # 偶数半数
            f = 构建冫循环赛配对方案冖冖非递归冫奇位移辻偶偏心对称乊仅限于偶数半数扌
        else:
            # 奇数半数
            f = 构建冫循环赛配对方案冖冖非递归冫缺一轮奇位移辻偶中心对称乊仅限于奇数半数扌
        return f(队伍数半数=队伍数半数)
    #end-def main():
    main4偶数半数冖冖对称 = 构建冫循环赛配对方案冖冖非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数扌
    main4偶数半数冖冖对称辻位移 = 构建冫循环赛配对方案冖冖非递归冫奇位移辻偶偏心对称乊仅限于偶数半数扌
    main4奇数半数冖冖对称辻位移 = 构建冫循环赛配对方案冖冖非递归冫缺一轮奇位移辻偶中心对称乊仅限于奇数半数扌
    main4合成 = 构建冫循环赛配对方案冖冖非递归冫惑缺一轮奇位移辻惑偶偏心偶中心对称扌
    main4合成.欤算法完整 = True
    return (main4偶数半数冖冖对称, main4偶数半数冖冖对称辻位移, main4奇数半数冖冖对称辻位移, main4合成)
def __():
    mk_fs_ = _构建冫循环赛配对方案冖冖非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数扌
    fs = mk_fs_()
    for f in fs:
        f.__doc__ = mk_fs_.__doc__
    assert fs[-1].欤算法完整
    for f in fs:
        _setdefault_properties_(f,欤算法完整=False, 欤算法递归=False, 欤算法蛮力=False)
    assert fs[-1].欤算法完整
    return fs
[构建冫循环赛配对方案冖冖非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数扌
,构建冫循环赛配对方案冖冖非递归冫奇位移辻偶偏心对称乊仅限于偶数半数扌
...
_版本名讠匴构造器
_版本名讠非递归构造器 = (dict
    #_setdefault_properties_():goto
    (蛮力最小化版=蛮力搜索冫循环对抗赛配对方案冖冖最小化扌
    ,非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数版=构建冫循环赛配对方案冖冖非递归冫偶偏心对称辻奇中心对称乊仅限于偶数半数扌
    ,非递归冫奇位移辻偶偏心对称乊仅限于偶数半数版=构建冫循环赛配对方案冖冖非递归冫奇位移辻偶偏心对称乊仅限于偶数半数扌
    ,非递归冫缺一轮奇位移辻偶中心对称乊仅限于奇数半数版=构建冫循环赛配对方案冖冖非递归冫缺一轮奇位移辻偶中心对称乊仅限于奇数半数扌
    ,非递归冫惑缺一轮奇位移辻惑偶偏心偶中心对称版=构建冫循环赛配对方案冖冖非递归冫惑缺一轮奇位移辻惑偶偏心偶中心对称扌
    #_setdefault_properties_():goto
    #
    #失败# ,非递归冫交错位移日字绕圈版=构建冫循环赛配对方案冖冖非递归冫交错位移日字绕圈扌
    ))
...
def 版本名讠算法信息扌(版本名, /):
    ...
    d
    obj = d[版本名]
    return _get_properties_(obj)
def 列表化冫所有版本信息扌():
    '-> [(版本名, 算法信息/{属性名:bool})]'
    列表纟版本名 = 列表化冫所有版本名扌()
    return [(版本名, 版本名讠算法信息扌(版本名)) for 版本名 in 列表纟版本名]
列表化冫所有版本信息扌()
    #测试:已初始化
def 紧凑文本化冫真值属性扌(属性名, 属性值, /):
    check_type_is(bool, 属性值)
    op = '-+'[属性值]
    return f'{op}{属性名}'
def 紧凑文本化冫算法信息扌(算法信息, /):
    return ','.join(紧凑文本化冫真值属性扌(属性名, 属性值) for 属性名, 属性值 in sorted(算法信息.items()))
def 紧凑文本化冫版本名辻算法信息扌(版本名, /):
    算法信息 = 版本名讠算法信息扌(版本名)
    s = 紧凑文本化冫算法信息扌(算法信息)
    return f'{版本名}:{s}'
def 打印冫所有版本信息扌():
    #for (版本名, 算法信息) in 列表化冫所有版本信息扌():
    for 版本名 in 列表化冫所有版本名扌():
        print(紧凑文本化冫版本名辻算法信息扌(版本名))
...
from seed.algo.round_robin_schedule import 检查冫圆满配对方案扌,列表化冫所有版本名扌
from seed.algo.round_robin_schedule import 列表化冫所有版本名扌,版本名讠算法信息扌,列表化冫所有版本信息扌,打印冫所有版本信息扌
from seed.algo.round_robin_schedule import 紧凑文本化冫真值属性扌,紧凑文本化冫算法信息扌,紧凑文本化冫版本名辻算法信息扌

from seed.algo.round_robin_schedule import 魖循环赛配对方案冖冖分而治之
from seed.algo.round_robin_schedule import 就地适配队伍号冫配对方案扌,组间配对冖冖相同扌,组间配对冖冖差一扌

from seed.algo.round_robin_schedule import 紧凑文本化冫配对方案扌,紧凑文本化冫合并轮纟二轮扌,合并冫二轮纟配对方案扌,压缩冫配对方案扌,标准压缩冫配对方案扌

from seed.algo.round_robin_schedule import 输出枚举冫循环赛配对方案扌,构建冫循环赛配对方案扌,枚举冫循环赛配对方案扌


from seed.algo.round_robin_schedule import 构建冫循环赛配对方案扌,构建冫循环赛配对方案冖冖易用接口扌,构建冫循环对抗赛配对方案巛变量环境扌

from seed.algo.round_robin_schedule import 枚举冫循环赛配对方案扌,枚举冫队伍数半数扌,枚举冫队伍数半数巛变量环境扌





from seed.algo.round_robin_schedule import *
