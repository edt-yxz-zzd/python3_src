#__all__:goto
r'''[[[
e ../../python3_src/seed/for_libs/for_pickle.py
update:
    view ../../python3_src/seed/types/Symbol.py

seed.for_libs.for_pickle
py -m nn_ns.app.debug_cmd   seed.for_libs.for_pickle -x
...
"pickle" module **is not secure**.
    <<==:
    unpickling
        ==>> import top_lvl-class, top_lvl-func
        ==>> call top_lvl-func to make instances
    Unpickler.find_class()
        -> class/func/.../???singleton???@object.__reduce__()

see:Restricting Globals
]]
[[
链表:会完蛋
...
call: __new__()
    to establish invariant
call: __getattribute__(),__getattr__(),__setattr__()
]]
[[
pickle  腌渍
marshal 整理,整顿
...
object.__reduce__()
    * -> qname4singleton
    * -> (mkr, args, ?may (state|dict)?, ?may iterator_xs?, ?may iterator_pairs?, ?may __setstate__?)
        # (mkr,args,kwargs,array,mapping,__setstate__)
        # [len <- [2..<=6]]
        * [has:__setstate__() or obj.__setstate__()]
            => state for __setstate__() or obj.__setstate__()
        * [no:__setstate__() or obj.__setstate__()]
            => dict to update obj.__dict__
        iterator_xs:obj.append/obj.extend
            ???list_comprehension
        iterator_pairs:obj.__setitem__
            ???dict_comprehension
...
view /sdcard/0my_files/unzip/py_doc/python-3.12.4-docs-text/library/copyreg.txt
===
...
[[[
view /sdcard/0my_files/unzip/py_doc/python-3.12.4-docs-text/library/pickle.txt
    view /sdcard/0my_files/unzip/py_doc/python-3.12.4-docs-html/_sources/library/pickle.rst.txt
...
import builtins
import pickle
from seed.helper._safe_eval__details import allows as _allows, forbids as _forbids
    #view ../../python3_src/seed/helper/safe_eval.py
    #view ../../python3_src/seed/helper/_safe_eval__details.py
...
class SafeUnpickler(pickle.Unpickler):
    def find_class(self, module, name, /):
        # Only allow safe classes from builtins.
        if not name[:1] == '_':
            if module == "builtins" and name in _allows:
                return getattr(builtins, name)
            elif module == "unicodedata" and not name[:1] == '_':
                import unicodedata
                return getattr(unicodedata, name)
        # Forbid everything else.
        raise pickle.UnpicklingError(f"global '{module}:{name}' is forbidden")

def safe_unpickle_loads(s, /):
    """Helper function analogous to pickle.loads()."""
    import io
    return SafeUnpickler(io.BytesIO(s)).load()
def safe_unpickle_load(ifile, /):
    """Helper function analogous to pickle.load()."""
    return SafeUnpickler(ifile).load()
