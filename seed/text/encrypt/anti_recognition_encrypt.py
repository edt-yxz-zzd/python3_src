...
++增加全角数字大写字母区#下划线？
++增加全角小写字母区
view /sdcard/0my_files/git_repos/python3_src/nn_ns/CJK/CJK_data/raw/U+FF00全角字符.txt
  # [:全角字符牜数字辻英文字母辻标点符号只保留部分]:goto
    #共225字符:部分标点符合162+全角空格1+全角数字10+全角大小写字母26*2
    #唯一一个高位面字符『W.Po@0x16FE2:𖿢:OLD CHINESE HOOK MARK』
    #不含『〇』
    r'！＂＃＄％＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾＿｀｛｜｝～｟｠￠￡￢￣￤￥￦〈〉⿰⿱⿲⿳⿴⿵⿶⿷⿸⿹⿺⿻、。〃〄〈〉《》「」『』【】〒〓〔〕〖〗〘〙〚〛〜〝〞〟〠〰〶〷〽〾゛゜゠・︐︑︒︓︔︕︖︗︘︙︰︱︲︳︴︵︶︷︸︹︺︻︼︽︾︿﹀﹁﹂﹃﹄﹅﹆﹇﹈﹉﹊﹋﹌﹍﹎﹏﹐﹑﹒﹔﹕﹖﹗﹘﹙﹚﹛﹜﹝﹞﹟﹠﹡﹢﹣﹤﹥﹦﹨﹩﹪﹫𖿢　０１２３４５６７８９ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚ'

==>>:
g_encrypt_decrypt_  :+ :'``1.618_""，。【  】〇『　』、！？﹏生也无知'"''"
'mod_add:19:``U.0W4f""，。【  】〇『\u3000』、！？ｇ畂亃昅砋\'\''

g_encrypt_decrypt_  :- :$'mod_add:19:``U.0W4f""，。【  】〇『\u3000』、！？ｇ畂亃昅砋\'\''
'``1.618_""，。【  】〇『\u3000』、！？﹏生也无知\'\''
...
==>>:
xxx:g_encrypt_decrypt_  :+ :'``1.618"" 生也无知'"''"
    KeyError: "'`'"
        ???怎么回事:新版py显示异常参数使用repr???

g_encrypt_decrypt_  :+ :'_ 1.618 生也无知'
'mod_add:19:a U.0W4 畀亁昃砉'

g_encrypt_decrypt_  :- :'mod_add:19:a U.0W4 畀亁昃砉'
'_ 1.618 生也无知'
    ...
    def __init__(sf, /, *区号讠单区配置简易表达, 不动区配置简易表达=''):
        ...
        def main():
            ...
            for 区号,单区配置简易表达 in enumerate(区号讠单区配置简易表达):
                ...
                区号 = -1
                列表纟卂起始位号丶数量丶起始字符编码点厈 = [*g(单区配置简易表达)]
                位数纟不动区 = sum(map(snd, 列表纟卂起始位号丶数量丶起始字符编码点厈))
                #列表纟卂起始字符编码点丶数量丶卂区号丶位移量臫位号厈厈.extend((pt,sz,(区号,pt-j)) for j,sz,pt in 列表纟卂起始位号丶数量丶起始字符编码点厈)
                    #位号无用，应该自动排序...
                def __():
            ...
            #==>>:
            区号讠位号讠位移量臸字符编码点#不含不动区
            列表纟卂起始字符编码点丶数量丶卂区号丶位移量臫位号厈厈#含不动区
            位数纟不动区
            ######################
            列表纟卂起始字符编码点丶数量丶卂区号丶位移量臫位号厈厈.sort()
...
if 1:
    ...
    #移动『﹏』
    #？允许『　』
    _punctuation_fullwidth_unsorted_chars = r'！＂＃＄％＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾＿｀｛｜｝～｟｠￠￡￢￣￤￥￦〈〉⿰⿱⿲⿳⿴⿵⿶⿷⿸⿹⿺⿻、。〃〄〈〉《》「」『』【】〒〓〔〕〖〗〘〙〚〛〜〝〞〟〠〰〶〷〽〾゛゜゠・︐︑︒︓︔︕︖︗︘︙︰︱︲︳︴︵︶︷︸︹︺︻︼︽︾︿﹀﹁﹂﹃﹄﹅﹆﹇﹈﹉﹊﹋﹌﹍﹎﹏﹐﹑﹒﹔﹕﹖﹗﹘﹙﹚﹛﹜﹝﹞﹟﹠﹡﹢﹣﹤﹥﹦﹨﹩﹪﹫𖿢'
    '　'
...
class 乸匴加密牜防识别牜文本氺自定义数据牜区内变换乊孤码牜恒状态牜模加(魖匴加密牜防识别牜文本氺自定义数据牜区内变换乊孤码):
    ...
    匾协议 = 'mod_add'
    #@override
    #ver1:区位配置 = 乸区位配置牜码元为字符编码点.构造冫区位配置巛多区配置简易表达扌('0..=9A..=Z::;_a..=z', r' +-#@%,.!?:;~$^&*[](){}<>|=\/')
        #[len(不动区)==29]
    #ver2:增加汉字区:
    #区位配置 = 乸区位配置牜码元为字符编码点.构造冫区位配置巛多区配置简易表达扌('0..=9A..=Z::;_a..=z::;一..=龥', ' '+punctuation_ascii_sorted_chars.translate(str.maketrans('', '', '_"' "'`")))
        #[len(不动区)==(1+32)-4==29]
    r'''[[[
    ...
    #]]]'''#'''
    #ver3:增加其他全角字符++允许引号
    区位配置 = 乸区位配置牜码元为字符编码点.构造冫区位配置巛多区配置简易表达扌(
    '0..=9A..=Z::;_a..=z::;一..=龥::;０..=９Ａ..=Ｚ::;﹏ａ..=ｚ'
    , ' '+punctuation_ascii_sorted_chars.replace('_', '')
    + '　'+_punctuation_fullwidth_unsorted_chars.replace('﹏', '').replace('𖿢', '')
    + '〇'
    )
        #del 『.translate(str.maketrans('', '', '_"' "'`"))』
        #[len(不动区)==1+(32-1)+1+(162-2)+1==194]
    assert 区位配置.位数纟不动区 == 194, 区位配置.位数纟不动区
